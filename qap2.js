 ...
 * Completed Problem Questions for Javascript
 *
...
function snake(value) {
  var underscoreValue = value.replaceAll(" ", "_").toLowerCase();
...
var src = "https://www.youtube.com/watch?v=ZVuToMilP0A";
var width = 20;
var controls = true;
...
function createVideo(src, width, controls) {
  if (
    ...
    (typeof controls == "undefined" || controls === false)
  ) {
    console.log('<video src="' + src.trim() + '"></video>');
  } else if (
    typeof src !== "undefined" &&
    typeof width !== "undefined" &&
    (controls === false || typeof controls == "undefined")
  ) {
    if (isNaN(width)) {
      throw "Error: Width needs to be an number!";
    } else {
      console.log('<video src="' + src.trim() + '", ' + width + "></video>");
    }
  } else if (
    typeof src !== "undefined" &&
    typeof width !== "undefined" &&
    ...
    if (isNaN(width)) {
      throw "Width needs to be an number!";
    } else {
      console.log(
        '<video src="' +
          src.trim() +
    ...
    console.log('<video src="' + src.trim() + '", ' + controls + "></video>");
  } else {
    throw "Needs URL!";
  }
}
...
function parseDateString(value) {
  ...
  if (value.match(/(\d{4})-(\d{2})-(\d{2})/) === null) {
    throw "Not a vaild format. Should be YYYY-MM-DD!";
  } else {
...
toDateString(parseDateString("2022-01-02"));
/*******************************************************************************
...
var Longitude = "-79.8976]";
var Latitude = "33.9755";

function normalizeCoord(value) {
  if (-90 > Longitude > 90) {
    newLatitude = Longitude.replace(/[^a-zA-Z0-9. -]/g, "");
    newLongitude = Latitude.replace(/[^a-zA-Z0-9. -]/g, "");
    return "(" + newLatitude.trim() + ", " + newLongitude.trim() + ")";
  } else {
    Latitude = Latitude.replace(/[^a-zA-Z0-9. -]/g, "");
    Longitude = Longitude.replace(/[^a-zA-Z0-9. -]/g, "");
    return "(" + Latitude.trim() + ", " + Longitude.trim() + ")";
  }
}

console.log(normalizeCoord());
/*******************************************************************************
...
const content = normalizeCoord();
fs.appendFile("coordinates.txt", content + ", ", (err) => {});
...
function generateLicenseLink(licenseCode, targetBlank) {
  var licenseCodeLink = licenseCode.slice(3).toLowerCase();
  var targetLink =
  ...
  var link =
    ...
    '/4.0/">';
  if (targetBlank == true) {
    if (licenseCodeLink == "by") {
      console.log(targetLink + "Creative Commons Attribution License</a>");
    } else if (licenseCodeLink == "by-nc") {
      console.log(
        targetLink + "Creative Commons Attribution-NonCommercial License</a>"
      );
    } else if (licenseCodeLink == "by-sa") {
      console.log(
        targetLink + "Creative Commons Attribution-ShareAlike License</a>"
      );
    } else if (licenseCodeLink == "by-nd") {
      console.log(
        targetLink + "Creative Commons Attribution-NoDerivs License</a>"
      );
    } else if (licenseCodeLink == "by-nc-sa") {
      console.log(
        targetLink +
          "Creative Commons Attribution-NonCommercial-ShareAlike License</a>"
      );
    } else if (licenseCode == "by-nc-nd") {
      console.log(
        targetLink +
      ...
      console.log(
        '<a target="_blank" href="https://choosealicense.com/no-permission/"> All Rights Reserved</a>'
      );
    }
  } else {
    if (licenseCodeLink == "by") {
      console.log(link + "Creative Commons Attribution License</a>");
    } else if (licenseCodeLink == "by-nc") {
      console.log(
        link + "Creative Commons Attribution-NonCommercial License</a>"
      );
    } else if (licenseCodeLink == "by-sa") {
      console.log(link + "Creative Commons Attribution-ShareAlike License</a>");
    } else if (licenseCodeLink == "by-nd") {
      console.log(link + "Creative Commons Attribution-NoDerivs License</a>");
    } else if (licenseCodeLink == "by-nc-sa") {
      console.log(
        link +
          "Creative Commons Attribution-NonCommercial-ShareAlike License</a>"
      );
    } else if (licenseCodeLink == "by-nc-nd") {
      console.log(
        link + "Creative Commons Attribution-NonCommercial-NoDerivs License</a>"
      );
    } else {
      console.log(
...
generateLicenseLink("CC-BY-NC-SA", true);
 ...
 *
 ******************************************************************************/
var trueArr = ["YES", "Y", "OUI", "O", "T", "TRUE", "VRAI", "V"];
var falseArr = ["NO", "N", "NON", "FALSE", "FAUX", "F"];

function pureBool(booleanValue) {
  if (isNaN(booleanValue)) {
    var booleanValueWord = booleanValue.toUpperCase();
    if (booleanValue == "true" || booleanValue == "false") {
      return booleanValue;
    } else if (trueArr.includes(booleanValueWord)) {
      return true;
    } else if (falseArr.includes(booleanValueWord)) {
      return false;
    } else {
      //      return "invalid";
      throw "invalid value";
...
function every(list1, list2, list3) {
  try {
    if ((pureBool(list1) && pureBool(list2) && pureBool(list3)) === true) {
      return true;
    } else {
      return false;
    }
...
function any(list1, list2, list3) {
  try {
    if (
      ((pureBool(list1) && pureBool(list2)) ||
        (pureBool(list1) && pureBool(list3)) ||
        (pureBool(list2) && pureBool(list3))) === true
    ) {
      return true;
    } else {
      return false;
...
function none(list1, list2, list3) {
  try {
    if ((pureBool(list1) && pureBool(list2) && pureBool(list3)) === false) {
      return true;
    } else {
      return false;
    }
...
function buildUrl(query, order, count, license) {
  ...
  var linkStart = "https://api.inaturalist.org/v2/observations";
  if (query == "none") {
    throw "needs a search string";
  } else {
    var encodeQuery = titleCase(query);
    var linkQuery = "?query='" + encodeURIComponent(encodeQuery) + "'";
  }
  if (1 <= count && count <= 50) {
    var linkCount = "&count=" + count;
  } else {
    throw "count needs to be in range 1-50";
  }
  if (orderLower == "ascending" || orderLower == "descending") {
    var linkOrder = "&order=" + orderLower;
  } else {
    throw "order needs to be in ascending or descending order";
  }
  if (licenses.includes(licenseLower)) {
    var linkLicense = "&license=" + licenseLower;
  } else {
    throw "not a valid license";
  }
  var link = linkStart + linkQuery + linkCount + linkOrder + linkLicense;
  console.log(link);
}

buildUrl("african hedgehog", "ascending", 50, "none");
